...
public final class NewYorkButton: UIButton {
    ...
    private func setup(title: String, style: Style) {
        ...
        case .default:
            ...
            backgroundColor = UIColor(hex: 0x1473FF)

        case .cancel:
            titleLabel?.font = UIFont.systemFont(ofSize: Const.fontSize)
            titleColor = UIColor.black
            backgroundColor = UIColor(hex: 0xF3F3F5)
    ...
    private func performHighlight(_ isHighlighted: Bool) {
        UIView.animate(withDuration: Const.animationDuration) { [weak self] in
            if self?.style == .cancel {
                let color = isHighlighted ? NewYorkDynamicColor.Background.button(isHighlighted) : UIColor(hex: 0xF3F3F5)
                self?.backgroundColor = color
            }else if self?.style == .default {
                let color = isHighlighted ? NewYorkDynamicColor.Background.button(isHighlighted) : UIColor(hex: 0x1473FF)
                self?.backgroundColor = color
            }else {
