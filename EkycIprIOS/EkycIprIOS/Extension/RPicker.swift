...
@objc open class RPicker: NSObject {
    ...
    @objc class func selectCountdown(title: String? = nil,
                          cancelText: String? = nil,
                          doneText: String = "Done",
                          datePickerMode: UIDatePicker.Mode = .countDownTimer,
                          style: RDatePickerStyle = .Wheel,
                          didSelectCountdown : ((_ interval: TimeInterval)->())?) {
        
        guard let vc = controller(title: title, cancelText: cancelText, doneText: doneText, datePickerMode: datePickerMode, selectedDate: Date(), minDate: nil, maxDate: nil, type: .countdown, style: style) else { return }
        
        vc.onCountDownSeleced = { interval in
            didSelectCountdown?(interval)
        }
    }
...
class RPickerController: UIViewController {
    ...
    var onCountDownSeleced : ((_ interval: TimeInterval) -> Void)?
    ...
    private func initialSetup() {
        ...
        if pickerType == .date || pickerType == .countdown {
    ...
    private lazy var datePicker: UIDatePicker = {
        ...
        if datePickerMode == .countDownTimer {
            picker.countDownDuration = 3600 //second
        }
        if #available(iOS 14, *) {
            if datePickerStyle == .Wheel {
    ...
    private lazy var toolBarView: UIView = {
        ...
        cancelButton.pinConstraints(barView, left: 8, top: 0, bottom: 0, width: buttonWidth)
    ...
    @objc func onDoneButton(sender : UIButton) {
        ...
        } else if pickerType == .countdown {
            onCountDownSeleced?(datePicker.countDownDuration)
        } else {
